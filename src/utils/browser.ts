...
const BrowserCompatible = (agent?: string) => {
  ...
  const isIE =
    ...
    browserAgent.indexOf("MSIE") > -1;
  // 判断是否IE的Edge浏览器 [百科详情 - https://baike.baidu.com/item/Microsoft%20Edge#8]
  // 判断是否是旧版内核Edge
  const isEdge =
    browserAgent.indexOf("Edge") > -1 || browserAgent.indexOf("Edg") > -1;
  // 判断是否是QQ浏览器10版本以上
  ...
  const isChromeBrowser =
    ...
    browserAgent.indexOf("AppleWebKit") > -1;
...
export const isIEBrowser = userAgent
  ? userAgent.indexOf("rv:11.0") > -1 || userAgent.indexOf("MSIE") > -1
  : false;
